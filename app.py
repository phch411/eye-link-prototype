...
class EyeLinkDB:
    ...
    def fetch_students(self, school_id):
        """[핵심] 로그인한 학교의 ID와 일치하는 학생만 필터링하여 호출"""
        try:
            # students 테이블에서 school_id가 일치하는 행만 가져옵니다.
...
class EyeLinkApp:
    ...
    def show_dashboard(self):
        """로그인한 학교의 데이터만 보여주는 대시보드"""
        user = st.session_state['user_info']
        current_school_id = user['school_id'] # 가입 시 설정한 아이디
        
        st.sidebar.title(f"🏫 {user['school_name']}")
        st.sidebar.write(f"ID: {current_school_id}")
        if st.sidebar.button("로그아웃"):
            st.session_state['logged_in'] = False
            st.session_state['user_info'] = None
            st.rerun()

        st.title("👁️ 실시간 아동 안전 모니터링")
        # [수정] 현재 로그인한 학교 ID를 넘겨 해당 학생만 가져옵니다.
        df = self.db.fetch_students(current_school_id)
        
        if not df.empty:
            st.map(df)
            st.subheader(f"📋 {user['school_name']} 관리 학생 리스트")
            st.dataframe(df, use_container_width=True)
        else:
            st.info(f"현재 '{user['school_name']}'에 등록된 학생 데이터가 없습니다.")
