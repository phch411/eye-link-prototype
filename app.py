...
class EyeLinkDB:
    ...
    def authenticate(self, u_id, pw):
        """[해결] 테이블명을 'users'로 다시 고정"""
        try:
            # 에러 메시지 힌트(PGRST205)에 따라 'users' 테이블을 조회합니다.
            ...
            with st.container(border=True):
                ...
                if st.button("로그인", use_container_width=True):
                    ...
                    if user:
                        ...
                        st.rerun()
                    else: st.error("정보를 찾을 수 없습니다. 아이디/비번을 확인해 주세요.")
            ...
            <script>
                function init() {{
                    ...
                    kakao.maps.load(function() {{
                        ...
                        {other_markers} {path_js} {blink_js}
                    }});
                }}
                init();
            </script>
